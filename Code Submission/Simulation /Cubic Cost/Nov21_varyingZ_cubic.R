# ==============================================================================
# TWO-STAGE SIMULATION - VARYING Z_J VERSION WITH CUBIC COSTS
...
run_single_simulation <- function(sim_id, n1j, n2j, J) {
  ...
  return(list(
    ...
    z_j_values = z_j_current  # Store the z_j values used in this simulation
  ))
}
...
for (scenario_idx in 1:nrow(scenario_grid)) {
  ...
  z_j_values <- lapply(parallel_results, function(x) x$z_j_values)
...
calculate_scenario_metrics <- function(scenario_idx, all_results, trueXopt, beta_true_int1, beta_true_int2, beta_z, x_min, x_max, n_sims) {
  ...
  # Table C metrics
  # Note: z_j now varies per simulation, so we use the average z_j for each simulation
  z_j_means <- sapply(result$z_j_values, mean)
...
write.csv(Table1A_intervention_effects, "Table1A_sim_cubic_varying_zj_highconf.csv", row.names = FALSE)
write.csv(Table1B_optimization_performance, "Table1B_sim_cubic_varying_zj_highconf.csv", row.names = FALSE)
write.csv(Table1C_confidence_and_outcomes, "Table1C_sim_cubic_varying_zj_highconf.csv", row.names = FALSE)
...
cat("\n=== SIMULATION COMPLETE ===\n")
cat("All results saved to CSV files with '_cubic_varying_zj' suffix\n")
